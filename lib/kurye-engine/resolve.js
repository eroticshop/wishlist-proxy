...
export function distancePrice(distKm, tariff) {
  ...
  // Son band SINIRLI (to != null) ve mesafe onu aşıyorsa => kurye kapsamı dışı.
  // Son band to:null ise açık uçlu (üst sınırsız) => asla kapsam dışı olmaz.
  if (maxTo != null && distKm > maxTo) return null;

  let total = base;
  for (const band of bands) {
    if (distKm <= band.from) break;
    const upper = band.to == null ? Infinity : band.to; // null = üst sınırsız son band
    total += (Math.min(distKm, upper) - band.from) * band.rate;
  }
...
// ---- Açık uçlu son band (to:null) — gerçek tarife ----
const openTariff = {
  bands: [
    { from: 0, to: 20, rate: 30 },
    { from: 20, to: 50, rate: 25 },
    { from: 50, to: null, rate: 22 }, // üst sınırsız
  ],
  base: 500, min: 300,
};
test('distancePrice: açık uçlu band 50km üstünde null DÖNMEZ', () => {
  assert.equal(distancePrice(60, openTariff), 2070); // 500 + 20*30 + 30*25 + 10*22
});
test('distancePrice: açık uçlu — çok uzak da hesaplanır', () => {
  assert.equal(distancePrice(100, openTariff), 500 + 20*30 + 30*25 + 50*22); // 3350
});
test('distancePrice: kısa mesafe base+min etkisi (base>min)', () => {
  assert.equal(distancePrice(5, openTariff), 650); // 500 + 5*30
});
test('distancePrice: 20 ve 50 sınırlarında süreklilik', () => {
  assert.equal(distancePrice(20, openTariff), 1100);
  assert.equal(distancePrice(50, openTariff), 1850);
});
